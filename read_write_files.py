...
Output formats : txt, xml, tsv, ces, arc, tmx, tei
...
def tei_add_header(tei, title, lang, source_desc):
    ...
    ET.SubElement(title_stmt, f"{{{TEI_NS}}}title").text = title
    ET.SubElement(title_stmt, f"{{{TEI_NS}}}author").text = "AIlign"
    publication = ET.SubElement(file_desc, f"{{{TEI_NS}}}publicationStmt")
    ET.SubElement(publication, f"{{{TEI_NS}}}p").text = "Generated by AIlign"
    source = ET.SubElement(file_desc, f"{{{TEI_NS}}}sourceDesc")
    ET.SubElement(source, f"{{{TEI_NS}}}p").text = source_desc
    profile = ET.SubElement(header, f"{{{TEI_NS}}}profileDesc")
    usage = ET.SubElement(profile, f"{{{TEI_NS}}}langUsage")
    ET.SubElement(usage, f"{{{TEI_NS}}}language", {"ident": lang})


def tei_add_sentence(parent, sent, sent_id, token_prefix, token_counter, corresp=""):
    paragraph = ET.SubElement(parent, f"{{{TEI_NS}}}p")
    sentence = ET.SubElement(paragraph, f"{{{TEI_NS}}}s", {f"{{{XML_NS}}}id": sent_id})
    word_parent = sentence
    if corresp:
        word_parent = ET.SubElement(sentence, f"{{{TEI_NS}}}seg", {"corresp": corresp})
    for tok in re.findall(r"\S+", sent):
        token_counter += 1
        word = ET.SubElement(word_parent, f"{{{TEI_NS}}}w", {f"{{{XML_NS}}}id": f"{token_prefix}_t{token_counter}"})
        word.text = tok
    return token_counter


def write_tei_file(output_path, title, lang, body_builder, source_desc):
    tei = ET.Element(f"{{{TEI_NS}}}TEI")
    tei_add_header(tei, title, lang, source_desc)
...
def write_alignable(sents, id_sents, intervals, index, output_dir, output_file, output_format):
    ...
    Arguments :
        ...
        output_format : str : "ces" or "arc" or "txt" or "tei"
    ...
    try:
        if output_format == "tei":
            ...
            prefix = tei_id_prefix(output_file, "sent")
            lang = prefix.split("_")[-1] if "_" in prefix else prefix
            ...
            def add_body(body):
                ...
                for interval in intervals:
                    ...
                    for i in range(i1, i2 + 1):
                        sent_id = id_sents[i] if id_sents[i] else f"{prefix}_s{i + 1}"
                        if not sent_id.startswith(prefix + "_"):
                            sent_id = f"{prefix}_s{i + 1}"
                        token_counter = tei_add_sentence(body, sents[i], sent_id, prefix, token_counter)

            write_tei_file(output_path, output_file, lang, add_body, "Alignable text with AIlign")
            return
...
def write_aligned_points(params, sents1, id_sents1, sents2, id_sents2, filtered_x, filtered_y, output_dir, output_file,
    ...
    Arguments :
        ...
        output_dir : str : the path of output dir
        output_file : str : the name of output file
        output_format : str : "tmx" or "ces" or "ids" or "txt" or "tei"
    ...
    if output_format == "tei":
        ...
        def source_sent_id(i):
            if input_format in ["ces", "arc", "xml"]:
                return id_sents1[i]
            return f"{prefix1}_s{i + 1}"

        def target_sent_id(i):
            if input_format in ["ces", "arc", "xml"]:
                return id_sents2[i]
            return f"{prefix2}_s{i + 1}"
        ...
        def add_body(body):
            ...
            for i in range(len(filtered_x)):
                if anchor:
                    ...
                    x = filtered_x[i]
                    y = filtered_y[i]
                corresp = " ".join([f"#{source_sent_id(x[j])}" for j in range(len(x))])
                for j in range(len(y)):
                    token_counter = tei_add_sentence(
                        body,
                        sents2[y[j]],
                        target_sent_id(y[j]),
                        prefix2,
        ...
        write_tei_file(output_path, output_file, l2, add_body, "Sentence alignment with AIlign")
        return
